...
export const BaseForm = ({
  ...
  <section
    className={`flex flex-col gap-3 rounded-md bg-white dark:bg-dark-bg-secondary p-6 pt-4 shadow dark:shadow-dark-border transition-opacity duration-200 ${className}`}
  >
    {children}
  </section>
);
    ...
    >
      <div className="flex items-center justify-between gap-4">
        <div className="flex grow items-center gap-2">
          <Icon className="h-6 w-6 text-gray-600 dark:text-dark-fg-muted" aria-hidden="true" />
          <input
            type="text"
            className="block w-full border-b border-transparent text-lg font-semibold tracking-wide text-gray-900 dark:text-dark-fg dark:bg-dark-bg-secondary outline-none hover:border-gray-300 dark:hover:border-dark-border hover:shadow-sm focus:border-gray-300 dark:focus:border-dark-border focus:shadow-sm"
            value={heading}
      ...
      {showForm && addButtonText && (
        <div className="mt-2 flex justify-end">
          <button
            ...
            className="flex items-center rounded-md bg-white dark:bg-dark-bg-tertiary py-2 pl-3 pr-4 text-sm font-semibold text-gray-900 dark:text-dark-fg shadow-sm ring-1 ring-inset ring-gray-300 dark:ring-dark-border hover:bg-gray-50 dark:hover:bg-dark-bg"
          >
            <PlusSmallIcon
              className="-ml-0.5 mr-1.5 h-5 w-5 text-gray-400 dark:text-dark-fg-muted"
              aria-hidden="true"
            />
            {addButtonText}
          </button>
...
export const FormSection = ({
  ...
  return (
    <>
      {idx !== 0 && (
        <div className="mb-4 mt-6 border-t-2 border-dotted border-gray-200 dark:border-dark-border" />
      )}
